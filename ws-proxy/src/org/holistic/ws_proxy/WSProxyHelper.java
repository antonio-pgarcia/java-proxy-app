...
public class WSProxyHelper {
     ...
     */
    public HttpURLConnection openurl(String strURL) throws Exception {
    ...
    public String get_postdata(HttpServletRequest req) throws Exception {
        ...
        log.debug("The POST parameters received are: " + m_strPostData);
    ...
    public void set_headers2urlconn(HttpServletRequest req, URLConnection objURLConn) throws Exception  {
        String m_strName;
        String m_strValue;
        for(Enumeration m_objElement = req.getHeaderNames(); m_objElement.hasMoreElements(); log.debug("ClientToEndPoint HEADER[" + m_strName + "] - VALUE[" + m_strValue + "]")) {
    ...
    private void headers_endpoint2client(HttpServletResponse resp, URLConnection objURLConn) throws Exception {
        ...
        for(String m_sHeaderValue= objURLConn.getHeaderField(m_iHeaderNumber); m_sHeaderName != null && m_sHeaderValue != null; m_sHeaderValue = objURLConn.getHeaderField(m_iHeaderNumber)) {
            if(m_sHeaderName.equalsIgnoreCase("Set-Cookie")) {
                ...
                if(m_sCookie != null && !m_sCookie.equals("")) {
                    log.debug("EndPoint Cookie: " + m_sCookie);
                ...
                if(!m_sHeaderName.equalsIgnoreCase("Content-Length") && !m_sHeaderName.equalsIgnoreCase("Transfer-Encoding")) {
                    if( m_sHeaderName.equalsIgnoreCase("Cache-Control") && m_sHeaderValue.equalsIgnoreCase("max-age=43200") ) {
                        ...
                        log.debug("EndPointToClient HEADER[" + m_sHeaderName + "] - VALUE[" + m_sHeaderValue + "]");
                    }
                }
            m_iHeaderNumber++;
    ...
    private void get_endpointstream(HttpServletResponse resp, HttpURLConnection objURLConn) throws Exception {
        int m_iIndex = 0;
